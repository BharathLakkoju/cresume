...
export function EvaluatorSection() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setActiveStage((current) => (current < stageCount - 1 ? current + 1 : current));
    }, 520);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      setError(parsed.error.issues[0]?.message ?? "Please correct the inputs and try again.");
      return;
    }
    ...
    try {
      ...
      setLatestResult(payload.result.detectedRole ?? "ATS evaluation", payload.result);
    ...
    <section id="evaluator" className="py-24">
      <div className="container">
        <FadeIn className="max-w-3xl">
          <p className="text-sm font-medium uppercase tracking-[0.24em] text-primary">Evaluator</p>
          ...
          <p className="mt-4 text-lg leading-8 text-muted-foreground">
            The evaluator validates file input, parses structured resume content, and returns a weighted ATS score with role-specific recommendations.
          </p>
        </FadeIn>
                ...
                <form className="space-y-5" onSubmit={onSubmit}>
                  <div className="space-y-2">
                    ...
                    <label
                      ...
                      className="flex cursor-pointer flex-col gap-3 rounded-[24px] border border-dashed border-border bg-[#fffaf2] p-5 transition hover:border-primary/50 hover:bg-white"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-foreground">Drop resume or browse</p>
                  ...
                  <div className="grid gap-3 rounded-[24px] border border-border bg-[#fffaf2] p-4 text-sm text-muted-foreground">
                    <div className="flex items-center gap-3">
                      <ShieldCheck className="h-5 w-5 text-primary" />
                      <span>Zod input validation on both client and API route</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Sparkles className="h-5 w-5 text-primary" />
                      <span>Local history now, Supabase sync ready after env setup</span>
                    </div>
                  </div>
                ...
                >
                  <Card className="panel-grid overflow-hidden bg-[linear-gradient(180deg,rgba(255,255,255,0.74)_0%,rgba(255,248,233,0.84)_100%)]">
                    ...
                    <CardContent className="grid gap-4 lg:grid-cols-2">
                      <div className="rounded-[26px] border border-border bg-white/75 p-6">
                        <p className="text-sm uppercase tracking-[0.16em] text-primary">Expected output</p>
                        <p className="mt-4 font-display text-6xl font-semibold text-foreground">72</p>
                        <p className="mt-2 text-muted-foreground">ATS score out of 100 with weighted fit analysis.</p>
                      </div>
                      <div className="space-y-4 rounded-[26px] border border-border bg-white/75 p-6">
                        <div>
                          <p className="font-medium text-foreground">Breakdown</p>
                          <p className="mt-1 text-sm text-muted-foreground">
                            Keyword fit, semantic match, skills, experience, formatting.
                          </p>
                        </div>
                        <div>
                          <p className="font-medium text-foreground">Action plan</p>
                          <p className="mt-1 text-sm text-muted-foreground">
                            Missing keywords, resume gaps, and role-specific improvement suggestions.
                          </p>
                        </div>
                        <div>
                          <p className="font-medium text-foreground">History</p>
                          <p className="mt-1 text-sm text-muted-foreground">
                            Stored locally now and designed for authenticated Supabase persistence.
                          </p>
                        </div>
                      </div>
                    </CardContent>
